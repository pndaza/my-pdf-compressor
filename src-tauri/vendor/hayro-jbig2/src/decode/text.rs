...
pub(crate) fn decode_with(
    ...
    // Guard against runaway decodes (denial-of-service / pathological streams),
    // but raised well above upstream's 10_000. The upstream limit rejected
    // legitimate high-resolution text scans — e.g. 7408x10535 @ 240ppi pages
    // routinely declare tens of thousands of symbol instances per page, and
    // were dropped with TooManyInstances. 2M keeps a finite cap while covering
    // any realistic scanned page; decode of a true worst case here is still
    // bounded (each instance is a cheap placement op).
